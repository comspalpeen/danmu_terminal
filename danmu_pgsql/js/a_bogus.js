...
class ABogus {
    ...
    generateAbogus(params, body) {
        ...
        const array1 = CryptoUtility.sm3ToArray(Uint8Array.from(paramsHash1));

        // Hash(Hash(body))
        const bodyHash1 = this.cryptoUtility.paramsToArray(body, true);
        const array2 = CryptoUtility.sm3ToArray(Uint8Array.from(bodyHash1));
...
function get_ab(dpf, ua) {
    try {
        const abogus = new ABogus(null, ua);
